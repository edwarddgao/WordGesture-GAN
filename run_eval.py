...
app = modal.App('wordgesture-gan-eval')
volume = modal.Volume.from_name('wordgesture-data')

# Use the cached image without wandb for faster startup
image = (
    modal.Image.debian_slim(python_version='3.11')
    .pip_install('torch>=2.0.0', 'numpy>=1.24.0', 'scipy>=1.10.0', 'matplotlib>=3.7.0')
)
...
def train_and_evaluate(num_epochs: int = 200):
    ...
    # (x, y) only
    print('Computing L2 Wasserstein (x,y)...')
    ...
    # (x, y, t)
    print('Computing L2 Wasserstein (x,y,t)...')
    ...
    # DTW Wasserstein
    print('Computing DTW Wasserstein...')
    def dtw(s1, s2):
        n, m = len(s1), len(s2)
